...
static const sparc_def_t *cpu_sparc_find_by_name(const char *name);
                               ...
                               target_ulong pc2)
{
    ...
    #ifdef GDB_ENABLED
    gen_op_gdb_verify (dc->last_pc, pc1);
    #endif
    gen_goto_tb (dc, 0, pc1, pc1 + 4);

    gen_set_label(l1);
    #ifdef GDB_ENABLED
    gen_op_gdb_verify (dc->last_pc, pc2);
    #endif
...
static inline void gen_generic_branch (DisasContext *dc,
    target_ulong npc1, target_ulong npc2)
{
    int l1, l2;
    ...
    #ifdef GDB_ENABLED
    gen_op_gdb_verify (dc->last_pc, npc1);
    #endif
    ...
    gen_set_label(l1);
    #ifdef GDB_ENABLED
    gen_op_gdb_verify (dc->last_pc, npc2);
    #endif
...
static void disas_sparc_insn(DisasContext * dc)
{
    ...
    #ifdef GDB_ENABLED
    if (dc->npc != JUMP_PC)
        gen_op_gdb_verify (tmp_physaddr, dc->npc);
    else
        dc->last_pc = tmp_physaddr;
    #endif    
